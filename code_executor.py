"""
SecureCodeExecutor: Safe Python Code Execution for Data Analysis

This module provides a secure environment for executing Python code generated by AI
for data analysis tasks. It implements strict security measures to prevent malicious
code execution while allowing safe data manipulation and visualization operations.
...
class SecureCodeExecutor:
    ...
    generated by AI models. It implements multiple security layers:
    
    1. Import restrictions - Only allows safe data science libraries
    2. Keyword filtering - Blocks dangerous operations
    3. AST validation - Analyzes code structure for security risks
    4. Isolated execution - Runs code in controlled globals/locals
    
    Attributes:
        ALLOWED_IMPORTS (set): Whitelist of permitted import statements
        DANGEROUS_KEYWORDS (list): Blacklist of risky operations
        dataframe (pd.DataFrame): Currently loaded dataset
        execution_globals (dict): Safe execution environment
    """
    
    # Whitelist of allowed imports for data analysis
    ALLOWED_IMPORTS = {
        'pandas', 'pd', 'numpy', 'np', 'matplotlib.pyplot', 'plt', 
        'seaborn', 'sns', 'math', 'statistics', 'json', 're'
    }
    
    # Blacklist of dangerous operations that could compromise security
    DANGEROUS_KEYWORDS = [
    ...
    def __init__(self):
        ...
        Sets up the execution environment with safe built-ins and
        configures matplotlib for Streamlit compatibility.
        """
        ...
        # Create safe execution environment with restricted built-ins
    ...
    def load_dataframe(self, file_content: bytes, filename: str) -> bool:
        """
        Load a file into pandas DataFrame or as text content.
        ...
        try:
            ...
            if ext == 'csv':
                ...
                # Try to load JSON as DataFrame, fallback to text
                try:
                    self.dataframe = pd.read_json(io.BytesIO(file_content))
                    self.execution_globals['df'] = self.dataframe
                    return True
                except Exception:
                    # If JSON can't be converted to DataFrame, treat as text
                    self.dataframe = None
                ...
                # Try to parse as CSV/TSV, else treat as plain text
                try:
                    self.dataframe = pd.read_csv(io.BytesIO(file_content), sep=None, engine='python')
                    self.execution_globals['df'] = self.dataframe
                    return True
                except Exception:
                    # If can't parse as structured data, treat as text
                    self.dataframe = None
                    self.execution_globals['df'] = None
                    self.text_content = file_content.decode(errors='ignore')
                    return True
                ...
                # Store as text content since PDFs don't typically convert to DataFrames
                self.dataframe = None
                self.execution_globals['df'] = None
